...
static void ptrArraySwap(void *ptr, int i, int j) {
  void **arrayptr= ptr;
  void *tmp= arrayptr[i];
...
void sortPtrArray(void *ptrArray, int left, int right, Comparator compare) {
  sort(ptrArray, left, right, compare, ptrArraySwap);
}
